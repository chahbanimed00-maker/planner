/**
 * ULTIMATE 365-DAY TRANSFORMATION TRACKER v3.0 - IMPROVED
 * 
 ...
 * 3. Save and run generateTracker() once
...
const CONFIG = {
  TOTAL_DAYS: 365,
  TOTAL_WEEKS: 52,
  FINANCIAL_GOAL_USD: 5000,
  FINANCIAL_GOAL_TND: 15000,
  EXCHANGE_RATE: 3.0,
  CF_TARGET: 300,
  START_DATE: new Date(new Date().getFullYear(), new Date().getMonth(), new Date().getDate()),
  UNIVERSITY_END: new Date('May 25, 2026'),
  UNIVERSITY_HOURS: { START: 9, END: 17 },
  VACATION: {
    START: new Date(2025, 11, 22),
    END: new Date(2026, 0, 5)
  },
  EXAM_START: new Date(2026, 0, 6),
  MANDATORY_DAILY_REVIEW: 3,
  PUSHUP_DAILY_TARGET: 10,
  GITHUB: {
    USERNAME: 'HamaBytes',
    API_URL: 'https://api.github.com'
  },
  CODEFORCES_HANDLE: 'chahbani.mohammed',
  DISCORD_WEBHOOK: 'https://discordapp.com/api/webhooks/1445916088612421642/pfjMEBvMr-8To9wZqVjk1JLj-1lNYYi8KBB3qfxha7rwy5tz-oWZFd4LXrocHZWiA8FS',
  RETRY: {
    MAX_ATTEMPTS: 3,
    INITIAL_DELAY: 1000,
...
const Utils = {
  /**
   * Safe fetch with retry logic and exponential backoff
   */
  fetchWithRetry: async function(url, options = {}, attempts = 0) {
    try {
      ...
      if (code >= 200 && code < 300) {
        ...
        return { ok: true, code, text, json, error: null };
      }
      
      // Retry on 429 (rate limit) or 5xx errors
      if ((code === 429 || code >= 500) && attempts < CONFIG.RETRY.MAX_ATTEMPTS) {
        const delay = CONFIG.RETRY.INITIAL_DELAY * Math.pow(CONFIG.RETRY.BACKOFF_MULTIPLIER, attempts);
        Logger.warn(`Request failed with ${code}, retrying in ${delay}ms (attempt ${attempts + 1}/${CONFIG.RETRY.MAX_ATTEMPTS})`);
        Utilities.sleep(delay);
        return this.fetchWithRetry(url, options, attempts + 1);
      }
      
      return { ok: false, code, text, json: null, error: `HTTP ${code}` };
    } catch (error) {
      ...
      return { ok: false, code: 0, text: '', json: null, error: error.toString() };
    }
  },
   ...
   */
  getProperty: function(key, fallback = null) {
    try {
   ...
   * Date operations
   */
  formatDate: function(date, format = 'MMM d, yyyy') {
    try {
  ...
  dateOffset: function(days) {
    return new Date(CONFIG.START_DATE.getTime() + days * 24 * 60 * 60 * 1000);
  },
  ...
  validateUrl: function(url, requiredDomain = null) {
    try {
      const urlObj = new URL(url);
      if (requiredDomain && !urlObj.hostname.includes(requiredDomain)) {
        return null;
      }
      return url;
    } catch (error) {
   ...
   */
  batchSetValues: function(sheet, startRow, startCol, data) {
    if (!data || data.length === 0) return;
    try {
      sheet.getRange(startRow, startCol, data.length, data[0].length).setValues(data);
      return true;
    } catch (error) {
      Logger.error('Batch set values failed', error);
      // Fallback to individual cell writes
      for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < data[i].length; j++) {
          try {
            sheet.getRange(startRow + i, startCol + j).setValue(data[i][j]);
          } catch (e) {
            Logger.error(`Failed to set cell (${startRow + i}, ${startCol + j})`, e);
          }
        }
      }
      return false;
   ...
   * Sheet operations
   */
  getOrCreateSheet: function(ss, name, rows = 100, cols = 26) {
    try {
      let sheet = ss.getSheetByName(name);
      if (sheet) {
        sheet.clear({ contentsOnly: true });
        sheet.getBandings().forEach(b => b.remove());
        try { sheet.getDataRange().clearDataValidations(); } catch (e) { /* Ignore */ }
        
        // Ensure adequate dimensions
      ...
      if (newSheet.getMaxRows() > rows) {
        newSheet.deleteRows(rows + 1, newSheet.getMaxRows() - rows);
      }
      if (newSheet.getMaxColumns() > cols) {
        newSheet.deleteColumns(cols + 1, newSheet.getMaxColumns() - cols);
      }
      return newSheet;
    } catch (error) {
      Logger.error(`Failed to get/create sheet ${name}`, error);
      throw error;
    }
  }
};
...
const GitHub = {
  setupAPI: function() {
    ...
    const response = ui.prompt(
      '🔑 GitHub API Setup',
      'Enter your GitHub Personal Access Token (starts with ghp_):',
    ...
    Utils.setProperty('GITHUB_TOKEN', token);
    ui.alert('✅ GitHub authenticated!');
    this.syncCommits();
  },
  ...
  linkRepo: function() {
    ...
    const response = ui.prompt(
      '🔗 Link Repository',
      'Enter repo in format: username/repo',
      ui.ButtonSet.OK_CANCEL
    );
    ...
    Utils.setProperty('GITHUB_REPO', repo);
    ui.alert(`✅ Linked: ${repo}`);
    this.syncCommits();
  },

  syncCommits: async function() {
    const token = Utils.getProperty('GITHUB_TOKEN');
    const repo = Utils.getProperty('GITHUB_REPO');
    ...
    try {
      ...
      const url = `${CONFIG.GITHUB.API_URL}/repos/${repo}/commits?since=${since}&per_page=100`;
      
      const result = await Utils.fetchWithRetry(url, {
        headers: {
      ...
      if (dailyMaster) {
        Object.entries(commitsByDate).forEach(([dateStr, count]) => {
          const [year, month, day] = dateStr.split('-').map(Number);
          const commitDate = new Date(year, month - 1, day);
          
          const dateColumn = dailyMaster.getRange('B:B').getValues();
          for (let i = 0; i < dateColumn.length; i++) {
            if (dateColumn[i][0] instanceof Date &&
                dateColumn[i][0].toDateString() === commitDate.toDateString()) {
              dailyMaster.getRange(i + 1, 11).setValue(count);
              break;
            }
          }
        });
        
        SpreadsheetApp.getUi().alert(`✅ Synced ${commits.length} commits`);
      }
    } catch (error) {
...
const CodeForces = {
  setupAPI: function() {
    ...
    const handle = response.getResponseText().trim();
    if (!handle) return;
    
    Utils.setProperty('CODEFORCES_HANDLE', handle);
    this.fetchStats(handle);
    ui.alert(`✅ Connected: ${handle}`);
  },

  fetchStats: async function(handle = null) {
    handle = handle || Utils.getProperty('CODEFORCES_HANDLE', CONFIG.CODEFORCES_HANDLE);
    
    try {
      // Get user info
      const userResult = await Utils.fetchWithRetry(
        `https://codeforces.com/api/user.info?handles=${encodeURIComponent(handle)}`
      );
      
      if (userResult.ok && userResult.json?.status === 'OK') {
        const user = userResult.json.result[0];
        Utils.setProperty('CF_USER_STATS', JSON.stringify({
          handle: user.handle,
          rating: user.rating,
          rank: user.rank,
      ...
      // Get submissions
      const subsResult = await Utils.fetchWithRetry(
        `https://codeforces.com/api/user.status?handle=${encodeURIComponent(handle)}&from=1&count=1000`
      );
      
      if (subsResult.ok && subsResult.json?.status === 'OK') {
        const submissions = subsResult.json.result;
        ...
        submissions.forEach(sub => {
          if (sub.verdict === 'OK') {
            ...
            if (!solvedMap.has(key) || 
                sub.creationTimeSeconds < solvedMap.get(key).creationTimeSeconds) {
        ...
        const stats = {
          ...
          avgRating: solvedMap.size > 0 ? Math.round(
            Array.from(solvedMap.values())
              .reduce((sum, s) => sum + (s.problem.rating || 0), 0) / solvedMap.size
          ) : 0,
          lastUpdated: new Date().toISOString()
        };
        
        Utils.setProperty('CF_SUBMISSIONS_STATS', JSON.stringify(stats));
        Logger.info(`CF Stats: ${stats.totalSolved} solved, avg ${stats.avgRating} rating`);
      }
    } catch (error) {
      Logger.error('CodeForces stats fetch failed', error);
    }
  },

  syncProblems: async function() {
    const handle = Utils.getProperty('CODEFORCES_HANDLE', CONFIG.CODEFORCES_HANDLE);
    const ss = SpreadsheetApp.getActive();
    const cfSheet = ss.getSheetByName('💡 CODEFORCES');
    
    if (!cfSheet) {
      SpreadsheetApp.getUi().alert('❌ CodeForces sheet not found');
      return;
    }
    ...
    try {
      ...
      while (true) {
        const result = await Utils.fetchWithRetry(
          `https://codeforces.com/api/user.status?handle=${encodeURIComponent(handle)}&from=${from}&count=${pageSize}`
        );
        
        if (!result.ok || !result.json?.result?.length) break;
        
        allSubs = allSubs.concat(result.json.result);
        if (result.json.result.length < pageSize) break;
        from += pageSize;
      }
      
      // Filter to tracking window
      const startMs = CONFIG.START_DATE.getTime();
      const endMs = startMs + CONFIG.TOTAL_DAYS * 24 * 60 * 60 * 1000;
      ...
      allSubs.forEach(sub => {
        ...
        if (!acceptedMap.has(key) || 
            sub.creationTimeSeconds < acceptedMap.get(key).creationTimeSeconds) {
          acceptedMap.set(key, sub);
        }
      });
      
      const acceptedSubs = Array.from(acceptedMap.values())
      ...
      const existingUrls = cfSheet.getRange(10, 4, Math.max(0, cfSheet.getLastRow() - 9), 1)
        .getValues().flat();
      const existingKeys = new Set();
      
      existingUrls.forEach(url => {
        if (!url) return;
        const match = String(url).match(/problem.*?\/(\d+)\/([A-Za-z0-9]+)/);
        if (match) existingKeys.add(`${match[1]}-${match[2]}`);
      });
      ...
      acceptedSubs.forEach(sub => {
        ...
        rowsToAdd.push([
          existingCount + rowsToAdd.length + 1,
          new Date((sub.creationTimeSeconds || 0) * 1000),
          problem.name || `Problem ${key}`,
          `https://codeforces.com/problemset/problem/${problem.contestId || 0}/${problem.index || ''}`,
          problem.rating || 'N/A',
          (problem.tags || []).join(', '),
          '', '', '', '✅',
      ...
      if (rowsToAdd.length > 0) {
        ...
        Utils.batchSetValues(cfSheet, startRow, 1, rowsToAdd);
        cfSheet.getRange(startRow, 2, rowsToAdd.length, 1).setNumberFormat('yyyy-mm-dd');
        
        SpreadsheetApp.getUi().alert(`✅ Added ${rowsToAdd.length} problems`);
      }
    } catch (error) {
...
const GoogleCalendar = {
  setupCalendar: function() {
    ...
    const response = ui.prompt(
      ...
      'Enter your Google Calendar ID (from calendar settings):\nExample: your-email@gmail.com',
      ui.ButtonSet.OK_CANCEL
    );
    ...
    if (!calendarId || !calendarId.includes('@')) {
      ui.alert('❌ Invalid calendar ID');
      return;
    }
    
    Utils.setProperty('GOOGLE_CALENDAR_ID', calendarId);
    ui.alert(`✅ Calendar linked: ${calendarId}`);
  },

  syncTasksToCalendar: function() {
    const calendarId = Utils.getProperty('GOOGLE_CALENDAR_ID');
    if (!calendarId) {
      SpreadsheetApp.getUi().alert('⚠️ Calendar not configured. Setup first.');
      return;
    }
    ...
    try {
      ...
      const ss = SpreadsheetApp.getActive();
      const dailyMaster = ss.getSheetByName('📅 DAILY MASTER');
      if (!dailyMaster) throw new Error('Daily Master sheet not found');

      const data = dailyMaster.getDataRange().getValues();
      ...
      for (let i = 3; i < data.length; i++) {
        const taskDate = data[i][dateColIdx];
        const status = data[i][statusColIdx];
        const notes = data[i][notesColIdx] || '';
        ...
        try {
          const eventTitle = `📋 Day ${i-2} - ${status}`;
          ...
          const alreadyExists = existingEvents.some(e => 
            e.getTitle().includes(`Day ${i-2}`)
          );

          if (!alreadyExists) {
            const event = calendar.createEvent(eventTitle, startTime, endTime);
            if (notes) event.setDescription(notes);
            event.setColor(status.includes('Completed') ? '2' : '9');
            synced++;
          }
        } catch (e) {
          Logger.warn(`Failed to sync day ${i-2}: ${e}`);
        }
      }

      SpreadsheetApp.getUi().alert(`✅ Calendar sync complete! ${synced} events added.`);
    } catch (error) {
  ...
  scheduleMobileReminder: function(dayNum) {
    const calendarId = Utils.getProperty('GOOGLE_CALENDAR_ID');
    if (!calendarId) {
      SpreadsheetApp.getUi().alert('⚠️ Calendar not configured');
      return;
    }

    try {
      const calendar = CalendarApp.getCalendarById(calendarId);
      const targetDate = Utils.dateOffset(dayNum - 1);
      const startTime = new Date(targetDate);
      startTime.setHours(7, 0, 0, 0);
      const endTime = new Date(startTime);
      endTime.setHours(8, 0, 0, 0);

      const eventTitle = `🔔 Day ${dayNum} Reminder`;
      const event = calendar.createEvent(eventTitle, startTime, endTime);
      ...
      event.addPopupNotification(15);
      event.setColor('1');

      SpreadsheetApp.getUi().alert(`✅ Reminder scheduled for Day ${dayNum}`);
    } catch (error) {
      Logger.error('Schedule reminder failed', error);
      SpreadsheetApp.getUi().alert(`❌ Error: ${error.message}`);
    }
  },

  addWeeklyMilestones: function() {
    const calendarId = Utils.getProperty('GOOGLE_CALENDAR_ID');
    if (!calendarId) {
      SpreadsheetApp.getUi().alert('⚠️ Calendar not configured');
      return;
    ...
    try {
      ...
      for (let week = 1; week <= CONFIG.TOTAL_WEEKS; week++) {
        const sundayDate = new Date(CONFIG.START_DATE);
        sundayDate.setDate(sundayDate.getDate() + (week * 7) - 1);
        ...
        const eventTitle = `📊 Week ${week} Review & Planning`;
        const event = calendar.createEvent(eventTitle, startTime, endTime);
        event.setDescription(
          `✓ Review this week's progress\n` +
          `✓ Check Daily Master completion\n` +
          `✓ Analyze CF progress\n` +
          `✓ Plan next week's goals`
        );
        event.setColor('8');
        event.addPopupNotification(30);
        added++;
      }

      SpreadsheetApp.getUi().alert(`✅ Added ${added} weekly milestone events`);
    } catch (error) {
      Logger.error('Weekly milestones failed', error);
      SpreadsheetApp.getUi().alert(`❌ Error: ${error.message}`);
    }
  }
};
...
const Notifications = {
  send: function(title, message) {
    const webhook = Utils.getProperty('NOTIFICATION_TARGET', CONFIG.DISCORD_WEBHOOK);
    
    if (!webhook) {
      this.logToSheet(title, message);
      return;
    }
    
    try {
      if (webhook.includes('discord')) {
        this.sendDiscord(title, message, webhook);
      } else {
        this.logToSheet(title, message);
      }
    } catch (error) {
  ...
  sendDiscord: async function(title, message, webhookUrl) {
    const payload = {
      embeds: [{
        title: title,
        description: message,
        color: 3066993,
        footer: { text: "365-Day Tracker" },
        timestamp: new Date().toISOString()
      }],
      username: "📊 Tracker Bot"
    };
    
    await Utils.fetchWithRetry(webhookUrl, {
      method: 'post',
  ...
  logToSheet: function(title, message) {
    try {
      const ss = SpreadsheetApp.getActive();
      ...
      if (!sheet) {
        ...
        sheet.getRange(1, 1, 1, 4).setValues([['Timestamp', 'Title', 'Message', 'Status']])
          .setFontWeight('bold').setBackground('#4472C4').setFontColor('#ffffff');
      }
      
      const row = sheet.getLastRow() + 1;
      sheet.getRange(row, 1, 1, 4).setValues([[
        new Date(), title, message, 'LOGGED'
      ]]);
    } catch (error) {
...
// ======================
// SHEET BUILDERS (OPTIMIZED)
// ======================
const SheetBuilders = {
  createDashboard: function(ss) {
    const sheet = Utils.getOrCreateSheet(ss, '📊 DASHBOARD', 120, 8);
    sheet.setColumnWidths(1, 1, 240);
    sheet.setColumnWidths(2, 1, 200);
    
    // Header
    const header = sheet.getRange(1, 1, 1, 8);
    header.setValue('🌍 INTERNATIONAL FREELANCER DASHBOARD')
      .setFontSize(18).setFontWeight('bold')
      .setBackground('#D9E2F3').setHorizontalAlignment('center');
    
    // Key metrics
    const metrics = [
      ['Current Date', '=TODAY()'],
      ['Day Number', `=MAX(1,TODAY()-DATE(${CONFIG.START_DATE.getFullYear()},${CONFIG.START_DATE.getMonth()+1},${CONFIG.START_DATE.getDate()})+1)`],
      ['Days Remaining', `=MAX(0,${CONFIG.TOTAL_DAYS}-(TODAY()-DATE(${CONFIG.START_DATE.getFullYear()},${CONFIG.START_DATE.getMonth()+1},${CONFIG.START_DATE.getDate()})+1))`],
    ];
    
    let row = 3;
    metrics.forEach(([label, formula]) => {
      sheet.getRange(row, 1).setValue(label).setBackground('#F2F2F2');
  ...
  createDailyMaster: function(ss) {
    const columns = [
      'Day', 'Date', 'Day Name', 'Week', 'Phase',
      '☑️ Wake 6am', '☑️ Morning routine', '☑️ Plan day', '☑️ University prep',
      '🏫 UNIVERSITÉ', '☑️ Review (3h)', '☑️ Coding/CF', '☑️ Drone/Aero',
      '☑️ Physical', '☑️ Pushups', '☑️ Dinner', '☑️ Learning',
      'Deep Work (h)', 'CF Solved', 'Pushups', 'Sleep', 'Quality',
      'Energy', 'Cigarettes', 'Score', 'Status', 'Notes'
    ];
    
    const sheet = Utils.getOrCreateSheet(ss, '📅 DAILY MASTER', CONFIG.TOTAL_DAYS + 10, columns.length + 5);
    ...
    for (let day = 1; day <= CONFIG.TOTAL_DAYS; day++) {
      const currentDate = Utils.dateOffset(day - 1);
      const week = Math.floor((day - 1) / 7) + 1;
      const dayName = Utils.formatDate(currentDate, 'EEEE');
      const phase = this.getPhaseForDay(day);
      
      rows.push([
        day, currentDate, dayName, week, phase,
        false, false, false, false,  // Morning tasks
        '', false, false, false,      // Deep work
        false, false, false, false,   // Evening
        '', '', '', '', '',           // Tracking
        '', '', '', '⏸️', ''          // Status
      ]);
    }
    ...
    // Batch write
    Utils.batchSetValues(sheet, 4, 1, rows);
    sheet.getRange(4, 2, rows.length, 1).setNumberFormat('yyyy-mm-dd');
    
    // Setup checkboxes
    [6,7,8,9,11,12,13,15,16,17,18].forEach(col => {
      sheet.getRange(4, col, rows.length, 1).insertCheckboxes();
    });
...
function generateTracker() {
  ...
  try {
    ui.alert('🚀 Building tracker...\n\nThis may take 30-60 seconds.');
    
    SheetBuilders.createDashboard(ss);
    SheetBuilders.createDailyMaster(ss);
    ...
    ui.alert(
      ...
      'Use the 🚀 Tracker menu for all features.'
    );
  } catch (error) {
    Logger.error('Tracker generation failed', error);
    ui.alert(`❌ Error: ${error.message}\n\nCheck logs for details.`);
  }
}
// NOTE: onOpen is defined later once (complete menu)

// Menu functions
function setupGitHub() { GitHub.setupAPI(); }
function linkGitHubRepo() { GitHub.linkRepo(); }
function syncGitHub() { GitHub.syncCommits(); }
function setupCodeForces() { CodeForces.setupAPI(); }
function syncCodeForces() { CodeForces.syncProblems(); }
function setupGoogleCalendar() { GoogleCalendar.setupCalendar(); }
function syncCalendar() { GoogleCalendar.syncTasksToCalendar(); }
function addWeeklyMilestonesMenu() { GoogleCalendar.addWeeklyMilestones(); }
function scheduleMobileReminderPrompt() {
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt('Schedule Mobile Reminder', 'Enter day number (1-365):', ui.ButtonSet.OK_CANCEL);
  if (response.getSelectedButton() === ui.Button.OK) {
    const dayNum = parseInt(response.getResponseText(), 10);
    if (dayNum >= 1 && dayNum <= CONFIG.TOTAL_DAYS) {
      GoogleCalendar.scheduleMobileReminder(dayNum);
    } else {
      ui.alert('❌ Invalid day number');
    }
  }
}
...
function configureWebhook() {
  const ui = SpreadsheetApp.getUi();
  const response = ui.prompt('Configure Webhook', 'Enter Discord webhook URL:', ui.ButtonSet.OK_CANCEL);
  if (response.getSelectedButton() === ui.Button.OK) {
    const url = Utils.validateUrl(response.getResponseText().trim(), 'discord');
    if (url) {
      Utils.setProperty('NOTIFICATION_TARGET', url);
      ui.alert('✅ Webhook configured');
    } else {
...
function runSystemTest() {
  const results = [];
  results.push('🧪 SYSTEM TEST RESULTS\n');
  
  // Test 1: Configuration
  results.push(`✅ Configuration loaded`);
  results.push(`   Start Date: ${CONFIG.START_DATE.toDateString()}`);
  results.push(`   GitHub: ${CONFIG.GITHUB.USERNAME}`);
  results.push('');
  
  // Test 2: Properties
  const hasGitHub = Utils.getProperty('GITHUB_TOKEN') !== null;
  results.push(hasGitHub ? '✅ GitHub configured' : '⚠️ GitHub not configured');
  
  const hasCF = Utils.getProperty('CODEFORCES_HANDLE') !== null;
  results.push(hasCF ? '✅ CodeForces configured' : '⚠️ CodeForces not configured');
  results.push('');
  
  // Test 3: Sheets
  const ss = SpreadsheetApp.getActive();
  const requiredSheets = ['📊 DASHBOARD', '📅 DAILY MASTER'];
  const missingSheets = requiredSheets.filter(name => !ss.getSheetByName(name));
  
  if (missingSheets.length === 0) {
    results.push('✅ All required sheets exist');
  } else {
    results.push(`❌ Missing sheets: ${missingSheets.join(', ')}`);
  }
  results.push('');
  
  // Test 4: Notifications
  try {
    Notifications.send('🧪 Test', 'System test notification');
    results.push('✅ Notifications working');
  } catch (error) {
    results.push('❌ Notifications failed');
  }
  results.push('');
  
  results.push('═══════════════════════');
  results.push(missingSheets.length === 0 ? '🎉 All systems operational!' : '⚠️ Some issues found');
  results.push('═══════════════════════');
...
// ======================
// ADDITIONAL SHEET BUILDERS
// ======================
SheetBuilders.createIncomeTracker = function(ss) {
  const headers = [
    'Entry', 'Date', 'Client', 'Project', 'Hours', 'Amount (USD)', 
    'Amount (TND)', 'Method', 'Status', 'Invoice'
  ];
  
  const sheet = Utils.getOrCreateSheet(ss, '💰 INCOME', 200, headers.length + 2);
  
  // Header
  sheet.getRange(1, 1, 1, headers.length).setValues([headers])
    .setFontWeight('bold').setBackground('#FFD966')
    .setFontColor('#000000').setHorizontalAlignment('center');
  
  // Summary
  sheet.getRange(3, 1).setValue('Total USD');
  sheet.getRange(3, 2).setFormula('=SUM(F:F)').setNumberFormat('$#,##0.00');
  
  sheet.getRange(4, 1).setValue('Total TND');
  sheet.getRange(4, 2).setFormula(`=B3*${CONFIG.EXCHANGE_RATE}`).setNumberFormat('#,##0 "TND"');
  
  sheet.getRange(5, 1).setValue('Progress');
  sheet.getRange(5, 2).setFormula(`=B3/${CONFIG.FINANCIAL_GOAL_USD}`).setNumberFormat('0.0%');
  
  // Format data area
  sheet.getRange('B8:B200').setNumberFormat('yyyy-mm-dd');
  sheet.getRange('F8:F200').setNumberFormat('$#,##0.00');
  sheet.getRange('G8:G200').setNumberFormat('#,##0 "TND"');
  
  // Data validation
  sheet.getRange('I8:I200').setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(['Pending', 'Paid', 'Partial'])
      .build()
  );
...
SheetBuilders.createExpenseTracker = function(ss) {
  const headers = [
    'Entry', 'Date', 'Category', 'Description', 'Amount (TND)', 
    'Amount (USD)', 'Necessary?', 'Receipt', 'Method', 'Notes'
  ];
  
  const sheet = Utils.getOrCreateSheet(ss, '💸 EXPENSES', 200, headers.length + 2);
  
  // Header
  sheet.getRange(1, 1, 1, headers.length).setValues([headers])
    .setFontWeight('bold').setBackground('#FCE4D6')
    .setFontColor('#000000').setHorizontalAlignment('center');
  
  // Format
  sheet.getRange('B8:B200').setNumberFormat('yyyy-mm-dd');
  sheet.getRange('E8:E200').setNumberFormat('#,##0.00 "TND"');
  sheet.getRange('F8:F200').setNumberFormat('$#,##0.00');
  
  // Categories validation
  const categories = [
    'Rent', 'Dates', 'Cigarettes', 'Gym', 'Food/Groceries',
    'Transport', 'Lab Equipment', 'University', 'Misc'
  ];
  
  sheet.getRange('C8:C200').setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(categories)
      .build()
  );
  
  sheet.getRange('G8:G200').setDataValidation(
    SpreadsheetApp.newDataValidation()
...
SheetBuilders.createCodeForcesLog = function(ss) {
  const headers = [
    'Entry', 'Date', 'Problem', 'URL', 'Rating', 'Topics',
    'Time (min)', 'Attempts', 'Approach', 'Status', 'Language', 'Notes'
  ];
  
  const sheet = Utils.getOrCreateSheet(ss, '💡 CODEFORCES', 300, headers.length + 2);
  ...
  const stats = [
    ['Total Solved', '0'],
    ['Target', String(CONFIG.CF_TARGET)],
    ['Progress', '=B4/B5'],
    ['Avg Rating', '0']
  ];
  
  stats.forEach((row, i) => {
    sheet.getRange(4 + i, 1).setValue(row[0]).setFontWeight('bold');
    sheet.getRange(4 + i, 2).setValue(row[1]);
  });
  
  sheet.getRange(6, 2).setNumberFormat('0.0%');
  
  // Format data area
  sheet.getRange('B10:B300').setNumberFormat('yyyy-mm-dd');
...
SheetBuilders.createGameHub = function(ss) {
  const headers = [
    'Day', 'Date', 'GitHub', 'Work XP', 'CF XP', 'Gym XP',
    'Income XP', 'Total XP', 'Level', 'Badge', 'Streak', 'Notes'
  ];
  
  const sheet = Utils.getOrCreateSheet(ss, '🎮 GAME HUB', CONFIG.TOTAL_DAYS + 20, headers.length + 2);
  
  // Header
  sheet.getRange(1, 1, 1, headers.length).setValues([headers])
    .setFontWeight('bold').setBackground('#FFC000')
    .setFontColor('#000000').setHorizontalAlignment('center');
  
  // Hero stats
  sheet.getRange(3, 1).setValue('🏆 HERO STATUS').setFontSize(14).setFontWeight('bold');
  ...
  heroStats.forEach((row, i) => {
    sheet.getRange(5 + i, 1).setValue(row[0]).setFontWeight('bold');
    sheet.getRange(5 + i, 2).setFormula(row[1]);
  });
  
  // XP Guide
  sheet.getRange(9, 1).setValue('🔥 XP EARNING GUIDE').setFontSize(12).setFontWeight('bold');
  
  const xpGuide = [
    ['Activity', 'XP', 'Max'],
    ['Deep Work Hour', '+5', '40'],
    ['GitHub Commit', '+10', '100'],
    ['CF Problem', '+8', '80'],
    ['Gym Session', '+10', '20'],
    ['Client Income', '+30', '150']
  ];
  
  xpGuide.forEach((row, i) => {
    sheet.getRange(11 + i, 1, 1, 3).setValues([row]);
  });
  ...
  for (let day = 1; day <= CONFIG.TOTAL_DAYS; day++) {
    ...
    rows.push([
      day,
      `=DATE(${CONFIG.START_DATE.getFullYear()},${CONFIG.START_DATE.getMonth()+1},${CONFIG.START_DATE.getDate()})+${day-1}`,
      `=IFERROR(INDEX('📅 DAILY MASTER'!K:K,MATCH(B${row},'📅 DAILY MASTER'!B:B,0)),0)`,
      `=IFERROR(INDEX('📅 DAILY MASTER'!R:R,MATCH(B${row},'📅 DAILY MASTER'!B:B,0))*5,0)`,
      `=IFERROR(INDEX('💡 CODEFORCES'!A:A,MATCH(B${row},'💡 CODEFORCES'!B:B,0))*8,0)`,
      `=IFERROR(IF(INDEX('📅 DAILY MASTER'!N:N,MATCH(B${row},'📅 DAILY MASTER'!B:B,0))=TRUE,10,0),0)`,
      `=IFERROR(IF(INDEX('💰 INCOME'!F:F,MATCH(B${row},'💰 INCOME'!B:B,0))>0,30,0),0)`,
      `=SUM(D${row}:G${row})`,
      `=FLOOR(H${row}/50)+1`,
      `=IF(H${row}>=100,"🔥 Legend",IF(H${row}>=50,"🚀 Elite",IF(H${row}>=25,"🎯 Sharp","🌱 Trainee")))`,
      `=IF(H${row}>0,IF(H${row-1}>0,K${row-1}+1,1),0)`,
      ''
    ]);
  }
  
  Utils.batchSetValues(sheet, startRow, 1, rows);
...
SheetBuilders.createHealthSheet = function(ss) {
  const headers = [
    'Date', 'Gym?', 'Type', 'Exercises', 'Duration',
    'Pushups', 'Sleep Hours', 'Quality', 'Energy', 'Cigarettes', 'Notes'
  ];
  
  const sheet = Utils.getOrCreateSheet(ss, '🏋️ HEALTH', 400, headers.length + 2);
  ...
  // Stats
  const stats = [
    ['Gym Sessions', '=COUNTIF(B:B,"Yes")'],
    ['Avg Sleep', '=AVERAGE(G:G)'],
    ['Smoke-Free Days', '=COUNTIF(J:J,0)']
  ];
  
  stats.forEach((row, i) => {
    sheet.getRange(3 + i, 1).setValue(row[0]).setFontWeight('bold');
    sheet.getRange(3 + i, 2).setFormula(row[1]);
  });
  
  // Validations
  sheet.getRange('B8:B400').setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(['Yes', 'No'])
      .build()
  );
  
  sheet.getRange('H8:H400').setDataValidation(
    SpreadsheetApp.newDataValidation()
      .requireValueInList(['😴 Poor', '😐 OK', '😊 Good', '😴😴 Excellent'])
      .build()
  );
  
  sheet.getRange('I8:I400').setDataValidation(
    SpreadsheetApp.newDataValidation()
...
// Update generateTracker to include all sheets
function generateTrackerComplete() {
  const ss = SpreadsheetApp.getActive();
  const ui = SpreadsheetApp.getUi();
  
  try {
    const proceed = ui.alert('🚀 Building complete tracker...', 'This will (re)create many sheets and may take 1-2 minutes. A backup will be created. Continue?', ui.ButtonSet.YES_NO);
    if (proceed !== ui.Button.YES) return;
    ss.toast('Building complete tracker — creating backup first...', 'Tracker', 3);
    // Backup existing key sheets
    try {
      const sheetsToBackup = ['📅 DAILY MASTER','💡 CODEFORCES','🎮 GAME HUB','📊 DASHBOARD'];
      const backed = [];
      sheetsToBackup.forEach(name => {
        const sh = ss.getSheetByName(name);
        if (sh) {
          const blob = Utilities.newBlob(sh.getDataRange().getValues().map(r=>r.join('\t')).join('\n'), 'text/plain', `${name}-backup-${Date.now()}.txt`);
          const file = DriveApp.createFile(blob);
          backed.push(file.getId());
        }
      });
      if (backed.length) Utils.setProperty('LAST_BACKUP_IDS', JSON.stringify(backed));
    } catch (e) { Logger.warn('Backup before rebuild failed: ' + e); }
    ui.alert('🚀 Building complete tracker...\n\nThis may take 1-2 minutes.');
    
    // Build all sheets
    SheetBuilders.createDashboard(ss);
    SheetBuilders.createDailyMaster(ss);
    SheetBuilders.createIncomeTracker(ss);
    SheetBuilders.createExpenseTracker(ss);
    SheetBuilders.createCodeForcesLog(ss);
    SheetBuilders.createGameHub(ss);
    SheetBuilders.createHealthSheet(ss);
    ...
    ui.alert(
      ...
      '• Game Hub\n' +
      '• Health Tracker\n\n' +
      'Next: Setup integrations via menu'
    );
  } catch (error) {
    Logger.error('Tracker build failed', error);
    ui.alert(`❌ Build failed: ${error.message}\n\nCheck Apps Script logs for details.`);
  }
}

/**
 * Diagnose data-validations across all sheets and report cells that contain 'problem' or similar keywords.
 */
    ...
    try {
      ...
      for (let r = 0; r < validations.length; r++) {
        for (let c = 0; c < validations[r].length; c++) {
          ...
          if (!dv) continue;
          try {
            const criteria = dv.getCriteriaType ? dv.getCriteriaType() : null;
            const args = dv.getCriteriaValues ? dv.getCriteriaValues() : [];
            const text = JSON.stringify(args).toLowerCase();
            if (text.includes('problem') || text.includes('probl') || text.includes('cf') ) {
              report.push([sh.getName(), `${r+1}:${c+1}`, String(criteria), String(args)]);
            }
          } catch (e) { /* ignore per-cell errors */ }
        }
      }
    } catch (e) { Logger.warn('diagnoseValidations error for sheet ' + sh.getName() + ': ' + e); }
  });

  const outName = '🛠 VALIDATION DIAG';
  const out = Utils.getOrCreateSheet(ss, outName, Math.max(10, report.length + 5), 4);
  out.clear();
  out.getRange(1,1,1,4).setValues([['Sheet','Cell','Criteria','Values/Range']]).setFontWeight('bold');
  if (report.length) out.getRange(2,1,report.length,4).setValues(report);
  out.autoResizeColumns(1,4);
  SpreadsheetApp.getUi().alert('🔍 Validation diagnostic complete — see sheet: ' + outName);
}

/**
 * Remove validations that match keywords (destructive). Requires confirmation.
 */
function findAndRemoveProblemValidations() {
  const ui = SpreadsheetApp.getUi();
  const ok = ui.alert('⚠️ Remove Problematic Validations?', 'This will clear data-validations that reference "problem"/"CF" keywords. This is destructive and cannot be undone without a backup. Continue?', ui.ButtonSet.YES_NO);
      ...
      for (let r = 0; r < validations.length; r++) {
        for (let c = 0; c < validations[r].length; c++) {
          ...
          try {
            ...
            const text = JSON.stringify(args).toLowerCase();
            if (text.includes('problem') || text.includes('probl') || text.includes('cf')) {
              toClear.push({r:r+1,c:c+1});
            }
          } catch (e) {}
        }
      }
      toClear.forEach(cell => { try { sh.getRange(cell.r, cell.c).clearDataValidations(); removed++; } catch (e) {} });
    } catch (e) { Logger.warn('findAndRemoveProblemValidations error: ' + e); }
  });
  SpreadsheetApp.getUi().alert('✅ Removed ' + removed + ' problematic validation(s).');
}

/**
 * Restore the last backup (simple restore from saved text backups) — creates new sheets prefixed with RESTORE_
 */
function restoreLastBackup() {
  const ui = SpreadsheetApp.getUi();
  const idsJson = Utils.getProperty('LAST_BACKUP_IDS');
  if (!idsJson) { ui.alert('No backups found'); return; }
  const ids = [];
  try { JSON.parse(idsJson).forEach(id => ids.push(id)); } catch (e) { ui.alert('Invalid backup metadata'); return; }
  ...
  ids.forEach(id => {
    try {
      ...
      const ss = SpreadsheetApp.getActive();
      const name = 'RESTORE_' + file.getName().slice(0,40);
      const sh = ss.insertSheet(name);
      sh.getRange(1,1,lines.length, lines[0].length).setValues(lines);
    } catch (e) { Logger.warn('restoreLastBackup failed for id ' + id + ': ' + e); }
  });
  ui.alert('Restore complete (RESTORE_ sheets created)');
}
...
const Helpers = {
  ...
  sendDailyReminder: function() {
    const today = new Date();
    const dayNum = Math.floor((today - CONFIG.START_DATE) / (24 * 60 * 60 * 1000)) + 1;
    
    if (dayNum < 1 || dayNum > CONFIG.TOTAL_DAYS) return;
    
    const phase = SheetBuilders.getPhaseForDay(dayNum);
    ...
    Notifications.send(
      ...
      `• 3h mandatory review\n` +
      `• 2-3 CF problems\n` +
      `• 1h drone/project work\n` +
      `• Gym session\n\n` +
      `Open your tracker to check off tasks!`
    );
  },
  ...
  exportSheetToCSV: function(sheetName) {
    try {
      ...
      const blob = Utilities.newBlob(csv, 'text/csv', `${sheetName}_${Date.now()}.csv`);
      
      // Save to Drive
      const folder = DriveApp.getRootFolder();
      const file = folder.createFile(blob);
      
      SpreadsheetApp.getUi().alert(`✅ Exported to:\n${file.getUrl()}`);
      Logger.info(`Exported ${sheetName} to Drive`);
    } catch (error) {
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('🚀 Tracker')
    .addItem('🔨 Build Complete Tracker', 'generateTrackerComplete')
    .addSeparator()
    .addItem('🔑 GitHub Setup', 'setupGitHub')
    .addItem('🔗 Link GitHub Repo', 'linkGitHubRepo')
    .addItem('🔄 Sync GitHub', 'syncGitHub')
    .addSeparator()
    .addItem('📊 CodeForces Setup', 'setupCodeForces')
    .addItem('🔄 Sync CodeForces', 'syncCodeForces')
    .addSeparator()
    .addItem('📅 Google Calendar Setup', 'setupGoogleCalendar')
    .addItem('📋 Sync Tasks to Calendar', 'syncCalendar')
    .addItem('📊 Add Weekly Milestones', 'addWeeklyMilestonesMenu')
    .addItem('🔔 Schedule Day Reminder', 'scheduleMobileReminderPrompt')
    .addSeparator()
    .addItem('⚙️ Configure Webhook', 'configureWebhook')
    .addItem('🔔 Enable Daily Reminders', 'enableDailyReminders')
    .addSeparator()
    .addItem('🔍 Diagnose Validations', 'diagnoseValidations')
    .addItem('🧹 Remove Problem Validations', 'findAndRemoveProblemValidations')
    .addItem('💾 Backup All Sheets', 'backupAllSheets')
    .addItem('♻️ Restore Last Backup', 'restoreLastBackup')
    .addSeparator()
    .addItem('📤 Export to CSV', 'exportDataMenu')
    .addItem('🧪 Test System', 'runSystemTest')
    .addToUi();
}

function enableDailyReminders() {
  Helpers.scheduleDailyReminder();
  SpreadsheetApp.getUi().alert('✅ Daily reminders enabled!\n\nYou\'ll get a notification at 8 AM daily.');
}
...
function exportDataMenu() {
  ...
  const response = ui.prompt(
    '📤 Export Data',
    'Enter sheet name to export (e.g., "📅 DAILY MASTER"):',
    ui.ButtonSet.OK_CANCEL
  );
  
  if (response.getSelectedButton() === ui.Button.OK) {
    Helpers.exportSheetToCSV(response.getResponseText().trim());
  }
}

/**
 * Backup selected sheets to Drive and remember IDs for restore.
 */
function backupAllSheets() {
  ...
  sheetsToBackup.forEach(name => {
    ...
    try {
      ...
      const csv = data.map(r => r.map(c => (c===null||typeof c==='undefined') ? '' : String(c).replace(/"/g,'""')).map(v=>'"'+v+'"').join(',')).join('\n');
      const blob = Utilities.newBlob(csv, 'text/csv', `${name}-backup-${Date.now()}.csv`);
      const file = DriveApp.createFile(blob);
      backed.push(file.getId());
    } catch (e) { Logger.warn('backupAllSheets failed for ' + name + ': ' + e); }
  });
  if (backed.length) Utils.setProperty('LAST_BACKUP_IDS', JSON.stringify(backed));
  ui.alert('✅ Backup complete. ' + backed.length + ' files created.');
}
/**
